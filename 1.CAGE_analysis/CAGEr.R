...
filename_list=c("BamGal4.1_R1_ctss.txt","BamGal4.2_R1_ctss.txt","K1_ctss.txt","K2_ctss.txt")
...
mergeSamples(myCAGEset, mergeIndex = c(1,1,2,2), 
            mergedSampleLabels = c("ctrl", "KmgKD"))
...
scoreShift(myCAGEset, groupX = "ctrl", groupY = "KmgKD", testKS = TRUE, useTpmKS = TRUE) # computing step 2
...
write.table(shifting.promoters, "shifting_promoters_xctrl_yKmgKD.txt", sep="\t", quote=F, row.names = F, col.names = T)
