...
filename_list=c("BAMgal4-1-ACCAGT_S3_R1_001_ctts.txt","BAMgal4-2-TGCACG_S4_R1_001_ctts.txt","KmgKD-1-CAGCGT_S1_R1_001_ctts.txt","KmgKD-2-GATCAC_S2_R1_001_ctts.txt")

myCAGEset <- new( "CAGEset", genomeName     = "BSgenome.Dmelanogaster.UCSC.dm6"
               , inputFiles     = filename_list
               , inputFilesType = "ctss"
               , sampleLabels   = sub( "_ctts.txt", "", basename(filename_list))
)
...
mergeSamples(myCAGEset, mergeIndex = c(1,1,2,2), 
            mergedSampleLabels = c("BamG4", "KmgKD"))
...
write.table(tc, paste0("clusterTTS_",nm,".txt"), sep="\t", quote=F, row.names = F, col.names = T)
}
...
scoreShift(myCAGEset, groupX = "BamG4", groupY = "KmgKD", testKS = TRUE, useTpmKS = TRUE) # computing step 2
...
write.table(shifting.promoters, "shifting_tts_xBamG4_yKmgKD.txt", sep="\t", quote=F, row.names = F, col.names = T)
